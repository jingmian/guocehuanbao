                            ...
                            if(_b3=="next"){
                                ...
                                if(_b3=="last"){
                                    ...
                                    if(_b3=="manual"){
                                        ...
                                        bb.num=$("<input class=\"pagination-num\" type=\"text\" value=\"1\" size=\"2\" readonly='readonly'>").appendTo(tr).wrap("<td></td>");
            ...
            if($.isArray(_af.buttons)){
               ...
               /* if(btnLen<5){*/
                    for(var i=0;i<btnLen;i++){
                        ...
                        if(isPower){
                            //2、判断该code值是否有权限
                            var currCode=btn.code;
                            //调用iframe父组件控制菜单权限的方法menuPower1
                            var state=parent.menuPower1(currCode);
                            if(state==false){
                                continue;
                            }
                        }
                ...
                /*}else{
                    var pFlag=true;
                    var p=$('<strong class="btn-more"></strong>');
                    ...
                    for(var i=0;i<btnLen;i++){
                        ...
                        if(i<4){
                            ...
                            if(pFlag){
                                var a=$("<a href=\"javascript:;\" class='btn' id='datagridMore'>更多</a>").appendTo(div);
                                p.appendTo(div);
                                pFlag=false;
                            }
                            var a=$("<a href=\"javascript:;\" class='btn'></a>").appendTo(p);
                            a[0].onclick=eval(btn.handler||function(){
                            });
                            a.linkbutton($.extend({},btn,{plain:true}));
                    ...
                    $(_b0).on('click','#datagridMore',function(){
...
*/
    ...
    },beforePageText:"Page",afterPageText:"of {page}",displayMsg:"Displaying {from} to {to} of {total} items",nav:{first:{iconCls:"pagination-first iconfont icon-shangyiye",handler:function(){
        var _d9=$(this).pagination("options");
        if(_d9.pageNumber>1){
            $(this).pagination("select",1);
        }
    }},prev:{iconCls:"pagination-prev iconfont icon-qianyiye",handler:function(){
        var _da=$(this).pagination("options");
        if(_da.pageNumber>1){
            $(this).pagination("select",_da.pageNumber-1);
        }
    }},next:{iconCls:"pagination-next iconfont icon-xiayiye",handler:function(){
        var _db=$(this).pagination("options");
        var _dc=Math.ceil(_db.total/_db.pageSize);
        if(_db.pageNumber<_dc){
            $(this).pagination("select",_db.pageNumber+1);
        }
    }},last:{iconCls:"pagination-last iconfont icon-moye",handler:function(){
            ...
            for(var i=0;i<_69a.length;i++){
                ...
                for(var j=0;j<cols.length;j++){
                    ...
                    if(col.checkbox){
                        ...
                        $("<div class=\"datagrid-header-check\"></div>").html("<input type=\"checkbox\"/><div class='boxDiv iconfont icon-xuanzhong'></div>").appendTo(td);
                    }else{
                        if(col.field){
                            td.attr("field",col.field);
    ...
    var _827={render:function(_828,_829,_82a){
        ...
        for(var i=0;i<_839.length;i++){
            ...
            if(col){
                ...
                cc.push(col.checkbox?"class=\"datagrid-cell-check iconfont icon-xuanzhong\"":"class=\"datagrid-cell "+col.cellClass+"\"");
                cc.push(">");
                if(col.checkbox){
    ...
    $.fn.datagrid.defaults=$.extend({},$.fn.panel.defaults,{sharedStyleSheet:false,frozenColumns:undefined,columns:undefined,fitColumns:false,resizeHandle:"right",resizeEdge:5,autoRowHeight:true,toolbar:null,striped:false,method:"post",nowrap:true,idField:null,url:null,data:null,loadMsg:"Processing, please wait ...",emptyMsg:"",rownumbers:false,singleSelect:false,ctrlSelect:false,selectOnCheck:true,checkOnSelect:true,pagination:false,pagePosition:"bottom",pageNumber:1,pageSize:10,pageList:[10,20,30,40,50],queryParams:{},sortName:null,sortOrder:"asc",multiSort:false,remoteSort:true,showHeader:true,showFooter:false,scrollOnSelect:true,scrollbarSize:18,rownumberWidth:30,editorHeight:24,headerEvents:{mouseover:_6af(true),mouseout:_6af(false),click:_6b3,dblclick:_6b8,contextmenu:_6bb},rowEvents:{mouseover:_6bd(true),mouseout:_6bd(false),click:_6c4,dblclick:_6ce,contextmenu:_6d2},rowStyler:function(_86d,_86e){
        ...
        function datagridAjax(){
            if(getCookie('token')=='undefined'){
                ...
                $.ajax({
                    type: opts.method, url: opts.url, data: _86f, dataType: "json",
                        ...
                        /* if(getCookie('token')=='undefined'){
                             $.ajax({
                                 ...
                                 success:function(data){
                                     ...
                                     if (data.code == 0) {
                                         setCookie("token", data.data.token);
                                         xhr.setRequestHeader('Authorization', 'Bearer ' + data.data.token);  //设置消息头
                                     }
                                 },
                                 error:function(data){
